...
<template>
  <div>
    <FinanceListWorkspace title="Customers" subtitle="Manage customer credit, balances, and collection health" action-label="Add Customer" :metrics="metrics" :columns="columns" :records="records" :filters="['Active','Review']" detail-base="/receivables/customers" :insight="{title:'Customer balances are live',text:'Open balances are calculated from invoices and payments on the backend.'}" :current-page="currentPage" :total-pages="totalPages" :total-records="totalRecords" :loading="loading" @primary-action="showModal = true" @next-page="nextPage" @prev-page="prevPage" @go-to-page="goToPage" />
    ...
    <div v-if="showModal" class="fixed inset-0 z-[60] flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
          ...
          <button @click="saveCustomer" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-md hover:bg-indigo-700 transition-colors shadow-sm">Save Customer</button>
        </div>
      </div>
    </div>
  </div>
