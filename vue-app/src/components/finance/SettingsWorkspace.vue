...
<template>
  <section class="settings-workspace">
    <PageHeader :title="title" :subtitle="subtitle">
      <template #actions><button class="save"><i class="ri-save-line"></i> Save changes</button></template>
    </PageHeader>
    <div class="settings-layout">
      <nav><button v-for="(section,index) in sections" :key="section.title" :class="{active:index===0}"><i :class="section.icon"></i>{{ section.title }}</button></nav>
      <main>
        <article v-for="section in sections" :key="section.title" class="settings-card">
          ...
          <div class="field-grid">
            <label v-for="field in section.fields" :key="field.label" :class="{wide:field.wide}">
              ...
              <select v-if="field.type === 'select'"><option>{{ field.value }}</option></select>
              <div v-else-if="field.type === 'toggle'" class="toggle-row"><p>{{ field.value }}</p><button class="toggle" :class="{on:field.enabled}"><span></span></button></div>
